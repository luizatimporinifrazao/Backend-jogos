...
const jogos = require("./data/games.json");
...
//http://localhost:3000/api/jogos/aleatorio
...
app.get("/api/jogos/aleatorio", (req, res) => {
    ...
    const todasAsFotos = Object.values(jogos).flat();
    ...
    res.json({
        ...
        message: `http://10.106.208.22:${PORT}/fotos/${item}`
    });
});

//===================================
// ROTA 2 - Cachorro por nome
//===================================
//http://localhost:3000/api/jogos/doom

app.get("/api/jogos/:nome", (req, res) => {
    ...
    if (!jogos[nome]){
        res.status(404).json({
            ...
            message: `jogo "${nome}" não encontrada`
        });
        //encerra a execução da rota
        return;
    }
    //sortear uma foto da raça solicitada
    const item = sortear(jogos[nome]);
...
app.listen(PORT, '0.0.0.0',() => {
    ...
    console.log(`📂 acesse de outra maquina usando: http;//SEU_IP:${PORT}`);
